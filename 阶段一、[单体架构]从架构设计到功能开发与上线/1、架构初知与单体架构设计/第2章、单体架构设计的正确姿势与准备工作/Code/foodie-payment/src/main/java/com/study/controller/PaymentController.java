...
import com.study.pojo.bo.MerchantOrdersBO;
import com.study.pojo.vo.PaymentInfoVO;
import com.study.resource.AliPayResource;
import com.study.resource.WXPayResource;
import com.study.service.PaymentOrderService;
import com.study.utils.CurrencyUtils;
import com.study.utils.StudyJSONResult;
import com.study.utils.RedisOperator;
import com.study.wx.entity.PreOrderResult;
import com.study.wx.service.WxOrderService;
...
public class PaymentController {
	...
	public StudyJSONResult createMerchantOrder(@RequestBody MerchantOrdersBO merchantOrdersBO, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isBlank(merchantOrderId)) {
			return StudyJSONResult.errorMsg("参数[orderId]不能为空");
		}
		if (StringUtils.isBlank(merchantUserId)) {
			return StudyJSONResult.errorMsg("参数[userId]不能为空");
		}
		if (amount == null || amount < 1) {
			return StudyJSONResult.errorMsg("参数[realPayAmount]不能为空并且不能小于1");
		}
		if (payMethod == null) {
			return StudyJSONResult.errorMsg("参数[payMethod]不能为空并且不能小于1");
		}
		if (payMethod != PayMethod.WEIXIN.type && payMethod != PayMethod.ALIPAY.type) {
			return StudyJSONResult.errorMsg("参数[payMethod]目前只支持微信支付或支付宝支付");
		}
		if (StringUtils.isBlank(returnUrl)) {
			return StudyJSONResult.errorMsg("参数[returnUrl]不能为空");
		}
		...
		try {
			...
			e.printStackTrace();
			StudyJSONResult.errorException(e.getMessage());
		}

		if (isSuccess) {
			return StudyJSONResult.ok("商户订单创建成功！");
		} else {
			return StudyJSONResult.errorMsg("商户订单创建失败，请重试...");
		}
	}
	...
	public StudyJSONResult getPaymentCenterOrderInfo(String merchantOrderId, String merchantUserId) {

		if (StringUtils.isBlank(merchantOrderId) || StringUtils.isBlank(merchantUserId)) {
			return StudyJSONResult.errorMsg("查询参数不能为空！");
		}

		Orders orderInfo = paymentOrderService.queryOrderInfo(merchantUserId, merchantOrderId);

		return StudyJSONResult.ok(orderInfo);
	}
...
//	@GetMapping(value="/getWXPayQRCode")
	...
	public StudyJSONResult getWXPayQRCode(String merchantOrderId, String merchantUserId) throws Exception{
...
//				String total_fee = "1";	// 测试用 1分钱
			...
			return StudyJSONResult.ok(paymentInfoVO);
		} else {
			return StudyJSONResult.errorMsg("该订单不存在，或已经支付");
		}
	}
	...
	public StudyJSONResult goAlipay(String merchantOrderId, String merchantUserId) throws Exception{
...
//    	String total_amount = "0.01";	// 测试用 1分钱
		...
		return StudyJSONResult.ok(alipayForm);
	}
