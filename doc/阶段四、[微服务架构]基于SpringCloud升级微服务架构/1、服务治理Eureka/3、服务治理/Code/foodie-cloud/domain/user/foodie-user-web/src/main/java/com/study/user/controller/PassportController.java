...
import com.study.auth.service.pojo.Account;
import com.study.auth.service.pojo.AuthCode;
import com.study.auth.service.pojo.AuthResponse;
import com.study.controller.BaseController;
import com.study.pojo.StudyJSONResult;
import com.study.pojo.ShopcartBO;
import com.study.user.UserApplicationProperties;
import com.study.user.pojo.Users;
import com.study.user.pojo.bo.UserBO;
...
public class PassportController extends BaseController {
    ...
    private static final String UID_HEADER = "study-user-id";

    @ApiOperation(value = "用户名是否存在", notes = "用户名是否存在", httpMethod = "GET")
    @GetMapping("/usernameIsExist")
    public StudyJSONResult usernameIsExist(@RequestParam String username) {

        // 1. 判断用户名不能为空
        if (StringUtils.isBlank(username)) {
            return StudyJSONResult.errorMsg("用户名不能为空");
        }

        // 2. 查找注册的用户名是否存在
        boolean isExist = userService.queryUsernameIsExist(username);
        if (isExist) {
            return StudyJSONResult.errorMsg("用户名已经存在");
        }
    ...
    @PostMapping("/regist")
    public StudyJSONResult regist(@RequestBody UserBO userBO,
        ...
        if (userApplicationProperties.isDisableRegistration()) {
            ...
            return StudyJSONResult.errorMsg("当前注册用户过多，请稍后再试");
        }
        ...
        if (StringUtils.isBlank(username) ||
            ...
            return StudyJSONResult.errorMsg("用户名或密码不能为空");
        }
        ...
        if (password.length() < 6) {
            return StudyJSONResult.errorMsg("密码长度不能少于6");
        }

        // 3. 判断两次密码是否一致
        if (!password.equals(confirmPwd)) {
            return StudyJSONResult.errorMsg("两次密码输入不一致");
        }
...
//            }
    ...
    public StudyJSONResult login(@RequestBody UserBO userBO,
                                 HttpServletRequest request,
        ...
        if (userResult == null) {
            return StudyJSONResult.errorMsg("用户名或密码不正确");
        }
        ...
        if (!AuthCode.SUCCESS.getCode().equals(token.getCode())) {
            ...
            return StudyJSONResult.errorMsg("Token error");
        }
        // 将token添加到Header当中
        ...
        return StudyJSONResult.ok(userResult);
    }

    private StudyJSONResult loginFail(UserBO userBO,
                                      HttpServletRequest request,
                                      HttpServletResponse response,
                                      Throwable throwable) throws Exception {
        return StudyJSONResult.errorMsg("验证码输错了（模仿12306）" + throwable.getLocalizedMessage());
    }
    ...
    @PostMapping("/logout")
    public StudyJSONResult logout(@RequestParam String userId,
                                  HttpServletRequest request,
                                  HttpServletResponse response) {
