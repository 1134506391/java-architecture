...
//        return "login";
    ...
    public IMOOCJSONResult verifyTmpTicket(String tmpTicket,
        ...
        if (StringUtils.isBlank(tmpTicketValue)) {
            System.out.println("111"+tmpTicketValue);
            return IMOOCJSONResult.errorUserTicket("用户票据异常");
        }

        // 0. 如果临时票据OK，则需要销毁，并且拿到CAS端cookie中的全局userTicket，以此再获取用户会话
        if (!tmpTicketValue.equals(MD5Utils.getMD5Str(tmpTicket))) {
            System.out.println("222"+tmpTicketValue);
            return IMOOCJSONResult.errorUserTicket("用户票据异常");
        } else {
            // 销毁临时票据
            System.out.println("333"+tmpTicketValue);
        ...
        System.out.println("COOKIE_USER_TICKET"+COOKIE_USER_TICKET);
        System.out.println("REDIS_USER_TICKET"+REDIS_USER_TICKET);
        ...
        String userRedis = redisOperator.get(REDIS_USER_TOKEN + ":" + userId);
        if (StringUtils.isBlank(userRedis)) {
            System.out.println("555"+userRedis);
            return IMOOCJSONResult.errorUserTicket("用户票据异常");
        }
        System.out.println("666"+userRedis);
        // 验证成功，返回OK，携带用户会话
